...
namespace SanteDB.DisconnectedClient.SQLite.Migrations
{
    ...
    public class InitialQueueCatalog : IDbMigration
    {
        ...
        public bool Install()
        {
            ...
            var db = SQLiteConnectionManager.Current.GetConnection(ApplicationContext.Current?.ConfigurationManager.GetConnectionString(ApplicationContext.Current?.Configuration.GetSection<DataConfigurationSection>().MessageQueueConnectionStringName).ConnectionString);
            using (db.Lock())
            {
