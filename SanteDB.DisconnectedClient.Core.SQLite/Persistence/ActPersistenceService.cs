    ...
    public class ActPersistenceService : VersionedDataPersistenceService<Act, DbAct>
    {
        ...
        protected override Act CacheConvert(DbIdentified dataInstance, SQLiteDataContext context)
        {
            ...
            if (retVal != null)
            {
                ...
                // Exclude
                nameof(SanteDB.Core.Model.Acts.Act.Extensions),
                nameof(SanteDB.Core.Model.Acts.Act.Tags),
                nameof(SanteDB.Core.Model.Acts.Act.Identifiers),
                nameof(SanteDB.Core.Model.Acts.Act.Notes),
                nameof(SanteDB.Core.Model.Acts.Act.Policies)
                );
                return retVal;
            }
            else
        ...
        public virtual TActType ToModelInstance<TActType>(DbAct dbInstance, SQLiteDataContext context) where TActType : Act, new()
        {
            ...
            if (dbInstance.UpdatedTime != null)
            {
                ...
                retVal.SetPreviousVersion(new Act()
                {
