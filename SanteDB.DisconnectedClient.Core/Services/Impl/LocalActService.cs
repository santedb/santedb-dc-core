        ...
        IRepositoryService<TextObservation>
    {
        ...
        public TAct Insert<TAct>(TAct insert) where TAct : Act
        {
            ...
            if (insert.Relationships.Count > 0 || insert.Participations.Count > 0)
                ApplicationContext.Current.GetService<IQueueManagerService>().Outbound.Enqueue(Bundle.CreateBundle(insert), SynchronizationOperationType.Insert);
            else
                ApplicationContext.Current.GetService<IQueueManagerService>().Outbound.Enqueue(insert, SynchronizationOperationType.Insert);
        ...
        public TAct Obsolete<TAct>(Guid key) where TAct : Act
        {
            ...
            ApplicationContext.Current.GetService<IQueueManagerService>().Outbound.Enqueue(act, SynchronizationOperationType.Obsolete);
        ...
        public TAct Save<TAct>(TAct act) where TAct : Act
        {
            ...
            catch (KeyNotFoundException)
            {
                ...
                if (act.Relationships.Count > 0 || act.Participations.Count > 0)
                    ApplicationContext.Current.GetService<IQueueManagerService>().Outbound.Enqueue(Bundle.CreateBundle(act), SynchronizationOperationType.Insert);
                else
                    ApplicationContext.Current.GetService<IQueueManagerService>().Outbound.Enqueue(act, SynchronizationOperationType.Insert);
        ...
        public TAct Validate<TAct>(TAct data) where TAct : Act
        {
            ...
            if (details.Any(d => d.Priority == DetectedIssuePriorityType.Error))
                throw new SanteDB.Core.Exceptions.DetectedIssueException(details);
