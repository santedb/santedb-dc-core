...
namespace SanteDB.DisconnectedClient.Core.Synchronization
{
    ...
    public class RemoteSynchronizationService : ISynchronizationService, IDaemonService
    {
        ...
        /// Get the log entries
        /// </summary>
        public List<ISynchronizationLogEntry> Log => SynchronizationLog.Current.GetAll().OfType<ISynchronizationLogEntry>().ToList();
                ...
                try
                {
                    ...
                    for (int i = result.Count; i < result.TotalResults; i += result.Count)
                    {
                        ...
                        if (result != null)
                        {
                            ...
                            SynchronizationQueue.Inbound.Enqueue(result, SynchronizationOperationType.Sync);
